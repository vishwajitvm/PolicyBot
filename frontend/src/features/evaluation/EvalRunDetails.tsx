...
import { ActivitySquare, CheckCircle2, XCircle, Target, ChevronDown, ChevronUp, MessageSquare, BookOpen, AlertCircle, Bot, Clock, FileCode2 } from "lucide-react";
                        ...
                        run.details.map((detail: any, dIdx: number) => {
                          ...
                          return (
                            <div key={dIdx} className="flex flex-col gap-3 p-5 bg-slate-900/80 rounded-xl border border-white/5 relative">
                                  ...
                                  {parsed.expected && (
                                    <div className="flex items-start gap-2">
                                      ...
                                      <div className="text-xs text-slate-300 w-full">
                                        ...
                                        <div className="bg-black/30 p-3 rounded-md border border-white/5 max-h-32 overflow-y-auto custom-scrollbar cursor-text whitespace-pre-wrap">
                                          {parsed.expected}
                                        </div>
                                      </div>
                                    </div>
                              ...
                              {/* Reasoning */}
                              {detail.reason && (
                                <div className="mt-3 text-xs text-slate-300 bg-black/50 p-3 rounded-lg border border-amber-500/20 flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                                  <div className="flex items-start gap-2.5">
                                  ...
                                  {detail.trace && (
                                    <button 
                                      onClick={() => setActiveTrace(detail.trace)}
                                      className="flex items-center gap-1.5 px-3 py-1.5 bg-black/40 hover:bg-black/60 border border-white/10 rounded-lg text-slate-400 hover:text-white transition-colors shrink-0"
                                      title="View execution trace details"
                                    >
                                      <FileCode2 className="w-3.5 h-3.5" />
                                      <span className="font-medium text-[10px] uppercase tracking-wider">Trace Details</span>
                                    </button>
                                  )}
                                </div>
                              )}
