...
class RAGGraph:
    ...
    async def run(self, question: str, session_id: str | None = None, filters: dict | None = None) -> QueryResponse:
        ...
        vector_candidates = await self.retrieval.vector_search(normalized, filters, self.settings.top_k)
        trace.record("query_embedded", status="completed")
        trace.record("vector_candidates_retrieved", output_summary={"count": len(vector_candidates)})
        keyword_candidates = await self.retrieval.keyword_search(normalized, self.settings.top_k)
        trace.record("keyword_candidates_retrieved", output_summary={"count": len(keyword_candidates)})
        merged = self._merge(vector_candidates + keyword_candidates)
        trace.record("candidates_merged", output_summary={"count": len(merged)})
        reranked = self.reranker.rerank(merged, self.settings.rerank_top_k)
        trace.record("candidates_reranked", output_summary={"count": len(reranked)})
        fresh_candidates, freshness_decision = self.freshness.resolve(reranked)
        selected = fresh_candidates[: self.settings.rerank_top_k]
